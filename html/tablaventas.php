<?php 
    ...
    $seleccion="SELECT * FROM ventas WHERE eliminar=0";
	$resultado=mysqli_query($conexion, $seleccion);
?>
<div style="overflow-x:scroll;" class="bs-example4" data-example-id="contextual-table">
	<center><h2><label><b>Salidas</b></label></h2></center>
	<form method="POST" action="">
	    <button type="submit" class="verde" value="Regvnt" name="Regvnt" id="Regvnt">Registrar <i class="fa fa-plus-circle"></i></button>
	</form>
	<table class="table">
		<thead>
		    <tr class="success">
		...
		if (mysqli_num_rows($resultado)>0) {
		  while($filas=mysqli_fetch_assoc($resultado))
		  {
			...
			$seleccion2="SELECT * FROM ventasdetalle WHERE id_salida='".$filas['id']."'";
			...
			echo "<input type='hidden' name='IdFactura' id='IdFactura' value='".$filas['id']."'>";
			...
			echo "<form method='POST' action=''>";
			echo "<input type='hidden' name='IdFactura' id='IdFactura' value='".$filas['id']."'>";
			echo "<td><button type='submit' value='Elimvnt' name='Elimvnt' id='Elimvnt'> <img src='images/remove-user.png' width=25px height=25px> </button></td>";
			echo "</tr>";
			echo "</tbody>";
			echo "</form>";
		  }
